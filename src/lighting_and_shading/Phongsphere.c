...
double intersect_ray_with_sphere(t_point origin, t_vec4d direction, t_sphere sphere)
{
    ...
    double b = 2 * dot(ray_direction, sphere_to_ray);
    double c = dot(sphere_to_ray, sphere_to_ray) - 1; // Radius is 1 in local space
...
void render_sphere_with_phong_model(int width, int height)
{
    ...
    double pixel_size = 2.0 / height; // Assume field of view fits within [-1, 1]
    double half_width = aspect_ratio;
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            double t = intersect_ray_with_sphere(camera_position, ray_direction, sphere);
