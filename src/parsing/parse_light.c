...
void	parse_light(t_minirt *minirt, char *data, size_t *i)
{
	...
	minirt->scene.lights[idx].specs.point.intensity = scale_color(in_color,
		minirt->scene.lights[idx].ratio);
}
...
void	parse_spotlight(t_minirt *minirt, char *data, size_t *i)
{
	...
	minirt->scene.lights[idx].specs.spot.spot_angle *= ((float)PI / 180.f);
	in_color = parse_color(data, i, minirt);
	minirt->scene.lights[idx].specs.spot.intensity = scale_color(in_color,
		minirt->scene.lights[idx].ratio);
}
