...
static inline t_mat4d	mat4_from_quat(const t_vec4d q)
{
	...
	p.y = q.p * q.y;
	p.z = q.p * q.z;
	fill_row(&ret, 0, create_vec4d(1 - 2 * (y.y + z.z), 2 * (x.y - p.z), 2
		* (x.z + p.y)));
	fill_row(&ret, 1, create_vec4d(2 * (x.y + p.z), 1 - 2 * (x.x + z.z), 2
		* (y.z - p.x)));
	fill_row(&ret, 2, create_vec4d(2 * (x.z - p.y), 2 * (y.z + p.x), 1 - 2
		* (x.x + y.y)));
	fill_row(&ret, 3, create_vec4d(0, 0, 0, 1));
	return (ret);
}
