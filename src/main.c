...
// Making a scene with shadows test
	...
	t_camera	cam;
	cam = set_camera(PI/2);
	cam.transform = view_transform(create_point(-5, 4.5, -15), create_point(0, 1, 0), create_vec4d(0, 1, 0));
	...
	left.transform = mult_n_mat4d(2, scaling_mat(0.33, 0.33, 0.33), translation_mat(-1.5, 2.33, -0.75));
