...
int main(int ac, char **av)
{
	...
	t_camera cam;
	cam = set_camera(PI / 3);
	cam.transform = view_transform(create_point(-40, 10, -25), create_point(0, 1, 0), create_vec4d(0, 1, 0));
	...
	// s4.transform = mult_n_mat4d(2, scaling_mat(1.75, 1.75, 1.75), translation_mat(-10, 5, -2));
	...
	c1.material.pattern = &pat2;
	c1.material.pattern->p_scale = 1;
	c1.material.color = create_color(0, 255, 0);
	c1.material.reflect_s = 0;
	c1.transform = mult_n_mat4d(3, rotate_mat_x(-PI/2), scaling_mat(1, 1, 1), translation_mat(-14, 0, -3));
