...
void	intersect_cylinder(t_ray *r, t_object *o, t_itx_grp *xs)
{
	...
	abc.x = trfm_r.direction.x * trfm_r.direction.x + \
		trfm_r.direction.z * trfm_r.direction.z;
	if (abc.x < EPSILON)
		return ;
	abc.y = 2 * (trfm_r.origin.x * trfm_r.direction.x + \
		trfm_r.origin.z * trfm_r.direction.z);
	abc.z = trfm_r.origin.x * trfm_r.origin.x + \
		trfm_r.origin.z * trfm_r.origin.z - 1;
